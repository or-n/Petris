...
void main() {
	...
	color_ = color;
	if(nr == up)
		color_.rgb -= ((pos.y - blockPos.y) / blockSize.y) / 2.0 / 8.0;
	else if(nr == down)
		color_.rgb -= ((blockPos.y - pos.y) / blockSize.y) / 2.0;
	else
		color_.rgb -= .125 + (pos.y + blockSize.y - blockPos.y) / blockSize.y / 2.0 / 4.0;
	if(min(x, y) <= 5.0/12.0)
		color_.rgb *= max(.2, 2.0*min(x, y));
	else
		color_ = color;
} 