...
void Image(const sf::Texture& texture, const sf::Vector2f& size,
    const sf::Color& tintColor, const sf::Color& borderColor)
{
    ImGui::Image((void*)texture.getNativeHandle(), ImVec2(size.x, size.y), ImVec2(0, 0), ImVec2(1, 1),
                  ImColor(tintColor.r, tintColor.g, tintColor.b, tintColor.a), ImColor(borderColor.r, borderColor.g, borderColor.b, borderColor.a));
}
    ...
    const sf::Color& tintColor, const sf::Color& borderColor)
{
    ...
    ImGui::Image((void*)texture.getNativeHandle(), ImVec2(size.x, size.y), uv0, uv1,
                 ImColor(tintColor.r, tintColor.g, tintColor.b, tintColor.a), ImColor(borderColor.r, borderColor.g, borderColor.b, borderColor.a));
}
    ...
    float thickness)
{
    ...
    sf::Vector2f pos(ImGui::GetCursorScreenPos().x, ImGui::GetCursorScreenPos().y);
    ImVec2 A = ImVec2(a.x, a.y);
    A.x += pos.x;
    A.y += pos.y;
    ImVec2 B = ImVec2(b.x, b.y);
    B.x += pos.x;
    B.y += pos.y;
    draw_list->AddLine(A, B, ColorConvertFloat4ToU32(ImColor(color.r, color.g, color.b, color.a)), thickness);
}
                     ...
                     const sf::Color& bgColor, const sf::Color& tintColor)
{
    ...
    return ImGui::ImageButton((void*)texture.getNativeHandle(), ImVec2(size.x, size.y), uv0, uv1, framePadding,
                              ImColor(bgColor.r, bgColor.g, bgColor.b, bgColor.a), ImColor(tintColor.r, tintColor.g, tintColor.b, tintColor.a));
}
